...
class WfcImpl {
  ...
  onConnectionChanged(status) {
    if (
      !self.isLogined &&
      status === ConnectionStatus.ConnectionStatusConnected
    ) {
      self.isLogined = true
    }
    if (status === ConnectionStatus.ConnectionStatusConnected) {
      if (self.lastActiveTime === 0) {
        ...
        console.log(
          "to load default data estimate duration",
          estimatedTime
        )
        setTimeout(() => {
          ...
          console.log(
            "emit ConnectionStatusConnected after",
            estimatedTime
          )
    ...
    if (
      ...
      self.disconnect() // 区分退出的状态，修复退出后样式问题
    }

    if (status === ConnectionStatus.ConnectionStatusConnected) {
      ipcRenderer.send("start-op-server", {
        port: Config.OPEN_PLATFORM_SERVE_PORT,
      })
    }
  }
  ...
  onReceiveMessage(messages, hasMore) {
    ...
    msgs.forEach((m) => {
      ...
      if (
        msg.messageContent instanceof MarkUnreadMessageContent &&
        ...
        protoProxy.invoke(
          ...
          stringValue(markMsg.messageUid),
          stringValue(markMsg.timestamp)
        )
      }
      if (msg) {
  ...
  onDomainInfoUpdate(domInfoStr) {
    if (!self.isLogined) {
      return
    }

    let domainInfo = Object.assign(
      new DomainInfo(),
      JSON.parse(domInfoStr)
    )
    self.eventEmitter.emit(EventType.DomainInfoUpdate, domainInfo)
  }
  ...
  onFriendRequestUpdate(newRequests = "[]") {
    ...
    console.log(
      "friend request list update, new incomming requests",
      newRequests
    )
  ...
  onSecretMessageStartBurningListener(targetId, playedMessageId) {
    console.log(
      "onSecretMessageStartBurningListener",
      targetId,
      playedMessageId
    )
    self.eventEmitter.emit(
      EventType.SecretChatStartBurn,
      targetId,
      playedMessageId
    )
  }
  ...
  init(args = []) {
    ...
    // }
    // protoProxy.invoke('setAppName, pkg.name);
  ...
  attach() {
    ...
    if (
      this.connectionStatus === ConnectionStatus.ConnectionStatusConnected
    ) {
      this.isLogined = true
  ...
  setAppName(appName) {
    if (!appName || !appName.trim()) {
      console.error("appName can not be empty")
      return
    }
    protoProxy.invoke("setAppName", appName)
  }
  ...
  getUserInfo(userId, fresh = false, groupId = "") {
    ...
    let userInfoStr = protoProxy.invoke(
      "getUserInfo",
      userId,
      fresh,
      groupId
  ...
  async searchUserEx(domainId, keyword, searchType, page, successCB, failCB) {
    protoProxy.invokeAsync(
      "searchUserEx",
      domainId,
      keyword,
  ...
  searchGroups(keyword) {
    ...
    if (groupSearchResultListStr && groupSearchResultListStr.length > 0) {
      groupSearchResultListStr.forEach((g) => {
        groupSearchResultList.push(
          GroupSearchResult.fromProtoGroupSearchResult(g)
        )
      })
    }
  ...
  addGroupMembers(
    ...
    if (!notifyMessageContent) {
      notifyMessageContent = new AddGroupMemberNotification(
        self.getUserId(),
        memberIds
      )
    }
  ...
  getGroupMembersByType(groupId, memberType) {
    let memberIdsStr = protoProxy.invoke(
  ...
  modifyMyInfo(modifyMyInfoEntries, successCB, failCB) {
    protoProxy.invokeAsync(
      "modifyMyInfo",
      modifyMyInfoEntries[0].type,
      modifyMyInfoEntries[0].value,
      () => {
        if (successCB) {
          successCB()
        }
  ...
  getChatroomInfo(chatroomId, updateDt, successCB, failCB) {
    protoProxy.invokeAsync(
      ...
      (info) => {
        if (successCB) {
          let chatRoomInfo = Object.assign(
            new ChatRoomInfo(),
            JSON.parse(info)
          )
  ...
  createChannel(name, portrait, desc, extra, successCB, failCB) {
    protoProxy.invokeAsync(
      ...
      (info) => {
        if (successCB) {
          successCB(
            Object.assign(new ChannelInfo(), JSON.parse(info))
          )
        }
      },
      (errorCode) => {
      ...
      ci.menus = ci.menus.map((menu) => {
        ...
        if (menu.subMenus) {
          menu.subMenus = menu.subMenus.map((sm) =>
            Object.assign(new ChannelMenu(), sm)
          )
        }
        return menu
      })
    ...
    protoProxy.invokeAsync(
      ...
      (result) => {
        if (successCB) {
          ...
          tmp.forEach((channel) => {
            channels.push(
              Object.assign(new ChannelInfo(), channel)
            )
          })
  ...
  getConversationList(types, lines) {
    let conversationListStr = protoProxy.invoke(
    ...
    tmp.forEach((c) => {
      conversationInfoList.push(
        ConversationInfo.protoConversationToConversationInfo(c)
      )
    })
  ...
  getConversationInfo(conversation) {
    let convStr = protoProxy.invoke(
      "getConversationInfo",
      JSON.stringify(conversation)
    )
    return ConversationInfo.protoConversationToConversationInfo(
      JSON.parse(convStr)
    )
  }

  searchConversation(keyword, types = [], lines = []) {
    let result = protoProxy.invoke(
      "searchConversation",
      keyword,
      types,
      lines
    ...
    if (resultList && resultList.length > 0) {
      resultList.forEach((r) => {
        conversationSearchResult.push(
          ConversationSearchResult.fromProtoConversationSearchResult(
            r
          )
        )
      })
  ...
  getConversationUnreadCount(conversation) {
    let unreadCountStr = protoProxy.invoke(
  ...
  clearUnreadStatusBeforeMessage(conversation, messageId) {
    protoProxy.invoke(
      "clearMessageUnreadStatusBefore",
      JSON.stringify(conversation),
      messageId
    )
    let conversationInfo = self.getConversationInfo(conversation)
  ...
  getConversationRead(conversation) {
    let readedStr = protoProxy.invoke(
      "getConversationRead",
      JSON.stringify(conversation)
    )
  ...
  getConversationDelivery(conversation) {
    let deliveryStr = protoProxy.invoke(
      "getMessageDelivery",
      JSON.stringify(conversation)
    )
  ...
  getConversationFirstUnreadMessageId(conversation) {
    let messageId = protoProxy.invoke(
  ...
  getMessages(
    ...
    let protoMsgsStr = protoProxy.invoke(
      "getMessages",
      JSON.stringify(conversation),
      [],
      fromIndex,
  ...
  getMessageByUid(messageUid) {
    let mStr = protoProxy.invoke(
      "getMessageByUid",
      Long.fromValue(messageUid).toString()
    )
    return Message.fromProtoMessage(JSON.parse(mStr))
  }
  ...
  async sendMessageEx(
    ...
    if (
      ...
      Config.SEND_LOG_COMMAND &&
      Config.SEND_LOG_COMMAND.trim() &&
      message.messageContent.content === Config.SEND_LOG_COMMAND.trim()
    ) {
      ...
      if (latestLogFilePath) {
        ...
        let file = {
          ...
          name: latestLogFilePath.substring(
            latestLogFilePath.lastIndexOf("/") + 1
          ),
    ...
    if (
      ...
      this.isTcpShortLink() &&
      !this.isSupportBigFilesUpload()
    ) {
      console.error(
        "TCP短连接不支持内置对象存储，请把对象存储切换到其他类型"
      )
      failCB && failCB(-1)
      return
    }
    ...
    // 此处因为正式环境报错，code 4, 服务端配置无法轻易变更，故暂时不支持大文件上传
    if (
      message.messageContent instanceof MediaMessageContent &&
      (message.messageContent.localPath || message.messageContent.file) &&
      !message.messageContent.remotePath &&
      ...
      let insertedMsg = this.insertMessage(
        message.conversation,
      ...
      preparedCB &&
        preparedCB(insertedMsg.messageId, insertedMsg.timestamp)
      ...
      if (
        ...
        (!message.messageContent.file ||
          !(message.messageContent.file instanceof File))
      ) {
        let path = message.messageContent.file
          ? message.messageContent.file.path
          : message.messageContent.localPath
        let name = message.messageContent.file
          ? message.messageContent.file.name
      ...
      if (
        ...
        if (message.messageContent.localPath) {
          let thumbnail =
            await require("electron").nativeImage.createThumbnailFromPath(
              message.messageContent.localPath,
              { width: 200, height: 200 }
            )
          let dataUrl = thumbnail.toDataURL()
          message.messageContent.thumbnail = dataUrl.substring(
      ...
      this._uploadFile(
        insertedMsg,
        file,
        message.messageContent.mediaType,
        (uploaded, total) => {
          progressCB &&
            progressCB(uploaded, total, insertedMsg.messageId)
        },
        (remoteUrl) => {
          insertedMsg.messageContent.remotePath = remoteUrl
          this.updateMessageContent(
            insertedMsg.messageId,
            insertedMsg.messageContent
          )
          protoProxy.invokeAsync(
            "sendSavedMessage",
            insertedMsg.messageId,
            ...
            (messageUid, timestamp) => {
              ...
              console.log(
                "send saved msg success",
                messageUid,
                insertedMsg
              )
              successCB &&
                successCB(
                  insertedMsg.messageUid,
                  insertedMsg.timestamp
                )
            },
            (err) => {
              failCB && failCB(err)
    ...
    let msgStr = protoProxy.sendMessage(
      strConv,
      strCont,
      toUsers,
      0,
      ...
      (messageUid, timestamp) => {
        ...
        if (typeof successCB === "function") {
          successCB(
            Long.fromValue(messageUid),
            Long.fromValue(timestamp).toNumber()
          )
        }
      },
      (errorCode) => {
    ...
    if (typeof preparedCB === "function") {
      preparedCB(
        message.messageId,
        Long.fromValue(msgObj.timestamp).toNumber()
      )
    }
  }
  ...
  // 大文件上传
  _uploadFile(message, file, mediaType, progressCB, successCB, failCB) {
    let fileName = encodeURIComponent(file.name)
    let contentType = this._getContentType(mediaType, fileName)
    this.getUploadMediaUrl(
      fileName,
      mediaType,
      contentType,
      (uploadUrl, remoteUrl, backUploadUrl, serverType) => {
        uploadUrl =
          self.connectedToMainNetwork() || !backUploadUrl
            ? uploadUrl
            : backUploadUrl
        console.log(
          "getUploadMediaUrl",
        ...
        if (serverType === 0) {
          ...
          let token = ss[1]
          let key = ss[2]
          xhr = this._uploadXMLHttpRequest(
  ...
  async updateRemoteMessageContent(
    messageUid,
    messageContent,
    distribute,
    updateLocal,
  ...
  isGroupReceiptEnabled() {
    return protoProxy.invoke("isGroupReceiptEnabled")
  }
  ...
  isDisableSyncDraft() {
    return (
      this.getUserSetting(UserSettingScope.DisableSyncDraft, "") === "1"
    )
  }
  ...
  getConversationFileRecords(
    ...
    protoProxy.invokeAsync(
      ...
      JSON.stringify(conversation),
      fromUser,
      Long.fromValue(beforeUid).toString(),
      order,
      count,
      (frsStr) => {
  ...
  _objStrToFileRecordObj(obj) {
    ...
    fileRecord.conversation = new Conversation(
  ...
  async uploadMedia(
    ...
    if (fileOrData.path) {
      fileOrData = await fs.readFile(fileOrData.path, {
        encoding: "base64",
      })
    }
  ...
  getSecretChatInfo(targetId) {
    let secretChatInfoStr = protoProxy.invoke(
      "getSecretChatInfo",
      targetId
    )
  ...
  decodeSecretChatMediaData(targetId, mediaDatas) {
    return protoProxy.invoke(
      "decodeSecretChatMediaData",
      targetId,
      mediaDatas
    )
  }
  ...
  isEnableMesh() {
    return protoProxy.invoke("isEnableMesh")
  }

  getDomainInfo(domainId, refresh) {
    let domainInfoStr = protoProxy.invoke(
      "getDomainInfo",
      domainId,
      refresh
    )
  ...
  loadRemoteDomains(successCB, failCB) {
    protoProxy.invokeAsync(
      ...
      (domainInfosStr) => {
        let domainInfos = []
        if (domainInfosStr && domainInfosStr !== "") {
          let tmp = JSON.parse(domainInfosStr)
          tmp.forEach((d) => {
            let domainInfo = Object.assign(new DomainInfo(), d)
            domainInfos.push(domainInfo)
          })
        }

        if (successCB) {
          successCB(domainInfos)
        }
      },
      (errorCode) => {
        if (errorCode) {
  ...
  _setupEventListeners() {
    ...
    protoProxy.setProtoEventListener(
      "connectToServer",
      (host, ip, port) => {
    ...
    protoProxy.setProtoEventListener(
      "connectedToServer",
      (host, ip, port, mainNw) => {
        self.onConnectedToServer(host, ip, port, mainNw)
        self._connectedToMainNetwork = mainNw
      }
    )
    ...
    protoProxy.setProtoEventListener(
      "domainInfoUpdate",
      (domainInfoStr) => {
        self.onDomainInfoUpdate(domainInfoStr)
      }
    )
    ...
    protoProxy.setProtoEventListener(
      "groupMemberUpdate",
      (groupId, groupMembersStr) => {
        self.onGroupMemberUpdateListener(groupId, groupMembersStr)
      }
    )
    protoProxy.setProtoEventListener(
      "secretChatStateChange",
      (targetId) => {
        self.onSecretChatStateChangedListener(targetId)
      }
    )
    protoProxy.setProtoEventListener(
      "secretMessageStartBurn",
      (targetId, playedMessageId) => {
        self.onSecretMessageStartBurningListener(
          targetId,
          playedMessageId
        )
      }
  ...
  _getLatestLogFilePath() {
    ...
    const logFiles = fileNames
      .filter((fn) => fn.endsWith(".xlog"))
      .sort((a, b) => (a > b ? 1 : -1))
    return logFiles.length > 0
      ? appPath + "/" + logFiles[logFiles.length - 1]
      : null
  }
  ...
  _preloadDefaultData() {
    ...
    let conversationInfoList = self.getConversationList(
      [0, 1, 3],
      [0, 1, 2]
    )
    let groupIdIds = []
    ...
    for (let i = 0; i < uids.length / 2000; i++) {
      let infos = self.getUserInfos(
        uids.slice(2000 * i, (i + 1) * 2000),
        ""
      )
    ...
    let newGroupCount = 0
    let groupInfos = self.getGroupInfos(groupIdIds, false)
    newGroupCount = groupInfos.filter(
      (groupInfo) => groupInfo.updateDt === 0
    ).length
    ...
    // 超过一周没有活跃，就预加载数据
    if (
      new Date().getTime() / 1000 - self.lastActiveTime >
      7 * 24 * 60 * 60
    ) {
      // 每 2000 人 4 秒
  ...
  _preloadGroupMemberUserInfos(memberIds) {
    let friendIds = this.getMyFriendList()
    let toLoadIds = memberIds.filter(
      (uid) => friendIds.indexOf(uid) === -1
    )
    for (let i = 0; i < toLoadIds.length / 2000; i++) {
      self.getUserInfos(memberIds.slice(2000 * i, (i + 1) * 2000), "")
      console.log(
        "to preload groupMember userInfos",
        memberIds.slice(2000 * i, (i + 1) * 2000)
      )
    }
  }
