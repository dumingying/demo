...
export default class WfcCli {
    ...
    init() {
        ...
        var token =
            "dGx2nKj1jrLdHUt8B7Uwm19v/C0BJbR1tXySfG0Nf7mjpQVZX5Dk2d07UObJxNAzmTs6vVTsVsWMI3TIiCUd6SUKXNjdZHTT6N5NpIrSSKhVc9c60bJxAbmLCIRPVppKIkW0/0Lpx83B8Z0zlGMUrdadO3TUKaoh484yLKwoGac=";
        var host = "wildfirechat.net";
        var shortPort = 80;
        console.log(username, clientId, token, host);
...
vorpal.command("init", "init").action(function (args, callback) {
    wfcCli.init();
    callback();
});
...
vorpal
    .command(
        "setConversationTop <type> <target> <line> <isTop>",
        "set conversation top"
    )
    .action(function (args, callback) {
        let conv = new Conversation(args.type, args.target, args.line);
        wfcCli.wfc.setConversationTop(
            conv,
            args.isTop,
            () => {
...
vorpal
    .command(
        "setConversationSilent <type> <target> <line> <isSilent>",
        "set conversation silent"
    )
    .action(function (args, callback) {
        let conv = new Conversation(args.type, args.target, args.line);
        wfcCli.wfc.setConversationTop(
...
vorpal
    .command(
        "searchConversation <keyword> [type] [line]",
        "search conversation"
    )
    .action(function (args, callback) {
        this.log("search args", args);
        let result = wfcCli.wfc.searchConversation(
            args.keyword,
            [args.type],
            [args.line]
        );
...
vorpal
    .command(
        "getConversationInfo <type> <target> <line>",
...
vorpal
    .command(
        "geConversationUnreadCount <type> <target> <line>",
...
vorpal
    .command(
        "clearConversationUnreadStatus <type> <target> <line>",
        "clear conversation unread status"
    )
    .action(function (args, callback) {
        let conv = new Conversation(args.type, args.target, args.line);
...
vorpal
    .command(
        "getConversationMessages <type> <target> <line>",
...
vorpal
    .command(
        "searchConversationMessage <type> <target> <line> <keyword>",
