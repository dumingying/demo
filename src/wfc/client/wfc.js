...
export class WfcManager {
    ...
    constructor() {
        impl.eventEmitter = {
            emit: (ev, ...args) => {
                if (
                    ev === EventType.ConnectionStatusChanged ||
    ...
    getUserDisplayName(userId) {
        ...
        let displayName =
            userInfo.friendAlias || userInfo.displayName || userInfo.mobile;
        return this.resetUserMobile(displayName);
    }
    ...
    getGroupMemberDisplayName(groupId, userId, ignoreFriendAlias = false) {
        ...
        let name =
            ...
            userInfo.mobile ||
            userId;
        return this.resetUserMobile(name);
    }
    ...
    getUserDisplayNameEx(userInfo) {
        let exName =
            ...
            userInfo.mobile ||
            userInfo.uid;
        return this.resetUserMobile(exName);
    }
    ...
    getGroupMemberDisplayNameEx(userInfo) {
        let groupName =
            ...
            userInfo.mobile ||
            userInfo.uid;
        return this.resetUserMobile(groupName);
    }
    /**
     *  处理用户名称是手机号的名称转换成脱敏号码
     * @param {string} mobile
     * @return {string} 123****7890
     */
    resetUserMobile(mobile) {
        ...
        impl.getUserInfosEx(
            ...
            (userInfos) => {
                userInfos.forEach((u) => {
                    if (
                        !u.portrait ||
                        u.portrait.startsWith(Config.APP_SERVER)
                    ) {
    ...
    getMyFriendList(fresh = false) {
        ...
        let list = impl.getMyFriendList(fresh);
        if (secretStatus.value && secretConfig.value) {
            let friendIds = secretFriends.value.map((item) => item.userId);
            return list.filter((item) => !friendIds.includes(item));
        } else {
    ...
    getConversationList(types, lines) {
        ...
        let list = impl.getConversationList(types, lines);
        if (secretStatus.value && secretConfig.value) {
            let friendIds = secretFriends.value.map((item) => item.userId);
            let groupIds = secretGroups.value.map((item) => item.groupId);
            return list.filter((item) =>
                item.conversationType === 0
                    ? !friendIds.includes(item.target)
                    : !groupIds.includes(item.target)
            );
        } else {
    ...
    searchConversationEx(
        ...
        return impl.searchConversationEx(
            keyword,
            types,
            lines,
            startTime,
    ...
    searchConversationEx2(
        ...
        return impl.searchConversationEx2(
            keyword,
            types,
            lines,
            cntTypes,
     ...
     * @returns {UnreadCount}
     */
    getUnreadCount(types = [0, 1], lines = [0]) {
    ...
    getMessagesByTimestamp(
        ...
        return impl.getMessagesByTimestamp(
            conversation,
            contentTypes,
            timestamp,
            before,
     ...
     * @param failCB
     */
    loadRemoteConversationMessages(
        conversation,
        contentTypes,
        beforeUid,
        count,
     ...
     * @param failCB
     */
    loadRemoteConversationMessagesEx(
        conversation,
        contentTypes,
        beforeUid,
        count,
    ...
    searchMentionedMessages(conversation, keyword, desc, limit, offset) {
        return impl.searchMentionedMessages(
            conversation,
            keyword,
            desc,
            limit,
    ...
    searchMessageByTypes(
        ...
        return impl.searchMessageByTypes(
            conversation,
            keyword,
            contentTypes,
            desc,
    ...
    searchMessageByTypesAndTimes(
        ...
        return impl.searchMessageByTypesAndTimes(
            conversation,
            keyword,
            contentTypes,
            startTime,
    ...
    searchMentionedMessageEx(
        ...
        return impl.searchMentionedMessageEx(
            conversationTypes,
            lines,
            keyword,
            desc,
    ...
    updateRemoteMessageContent(
        ...
        impl.updateRemoteMessageContent(
            msgUid,
            messageContent,
            distribute,
            updateLocal,
    ...
    getConversationFileRecords(
        ...
        impl.getConversationFileRecords(
            conversation,
            fromUser,
            beforeMessageUid,
            order,
