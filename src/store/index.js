...
import { getItem, setItem, clear, unClearItem } from "@/ui/util/storageHelper";
...
// import ChannelInfoType from "@/wfc/model/channelInfo";
...
import useSecretTokenStore from "./secretStore";
import { generateUUID } from "../ui/util/idGenerate";

import Md5 from "crypto-js/md5";

import { getGroupInfos, generateGroupPortrait, getUpload2Minio } from "@/axios";
...
let store = {
    ...
    init(isMainWindow) {
        ...
        conferenceState = conferenceStore.value;
        ...
        store.state.conference = conferenceState;
        ...
        wfc.eventEmitter.on(EventType.UserInfosUpdate, (userInfos) => {
            console.log(
                "store UserInfosUpdate",
                userInfos,
                miscState.connectionStatus
            );
        ...
        wfc.eventEmitter.on(
            ...
            (groupId, members) => {
                ...
                this._reloadGroupConversationIfExist([
                    new NullGroupInfo(groupId),
                ]);
                // this._loadFavGroupList();
                // TODO 其他相关逻辑
            }
        ...
        wfc.eventEmitter.on(EventType.ReceiveMessage, (msg, hasMore) => {
            ...
            if (
                miscState.isMainWindow &&
                !this.isConversationInCurrentWindow(msg.conversation)
            ) {
                return;
            }

            if (!hasMore) {
                this._reloadConversation(msg.conversation, true, msg);
            }
            if (
                conversationState.currentConversationInfo &&
                msg.conversation.equal(
                    conversationState.currentConversationInfo.conversation
                )
            ) {
                if (
                    msg.messageContent instanceof DismissGroupNotification ||
                    (msg.messageContent instanceof
                        KickoffGroupMemberNotification &&
                        msg.messageContent.kickedMembers.indexOf(
                            wfc.getUserId()
                        ) >= 0) ||
                    (msg.messageContent instanceof QuitGroupNotification &&
                        msg.messageContent.operator === wfc.getUserId())
                ) {
                ...
                let lastTimestamp = 0;
                let msgListLength =
                    conversationState.currentConversationMessageList.length;
                if (msgListLength > 0) {
                    lastTimestamp =
                ...
                let msgIndex =
                    conversationState.currentConversationMessageList.findIndex(
                        (m) => {
                            return (
                                m.messageId === msg.messageId ||
                                (gt(m.messageUid, 0) &&
                                    eq(m.messageUid, msg.messageUid))
                            );
                        }
                    );
                if (msgIndex > -1) {
                    // FYI: https://v2.vuejs.org/v2/guide/reactivity#Change-Detection-Caveats
                    conversationState.currentConversationMessageList.splice(
                        msgIndex,
                        1,
                        msg
                    );
                ...
                // 处理一下路径
                msg.messageContent.remotePath &&
                    (msg.messageContent.remotePath = this._checkRemotePath(
                        msg.messageContent.remotePath
                    ));
            ...
            if (
                ...
                if (
                    msg.conversation.type !== 2 &&
                    miscState.isPageHidden &&
                    (miscState.enableNotification ||
            ...
            if (conversationState.currentConversationInfo) {
                ...
                if (
                    ...
                    if (conversationState.currentConversationMessageList) {
                        let lastTimestamp = 0;
                        conversationState.currentConversationMessageList =
                            conversationState.currentConversationMessageList.map(
                                (msg) => {
                                    if (eq(msg.messageUid, messageUid)) {
                                        let newMsg =
                                            wfc.getMessageByUid(messageUid);
                                        this._patchMessage(
                                            newMsg,
                                            lastTimestamp
                                        );
        ...
        wfc.eventEmitter.on(EventType.UserOnlineEvent, (userOnlineStatus) => {
            ...
            // 更新在线状态
            contactState.friendList = this._patchAndSortUserInfos(
        ...
        wfc.eventEmitter.on(EventType.MessageDeleted, (messageUid) => {
            ...
            if (conversationState.currentConversationInfo) {
                if (conversationState.currentConversationMessageList) {
                    conversationState.currentConversationMessageList =
                        conversationState.currentConversationMessageList.filter(
                            (msg) => !eq(msg.messageUid, messageUid)
                        );
                }
            }
            this.updateTray();
        ...
        wfc.eventEmitter.on(EventType.SecretChatMessageBurned, (messageIds) => {
            ...
            if (conversationState.currentConversationInfo) {
                if (conversationState.currentConversationMessageList) {
                    conversationState.currentConversationMessageList =
                        conversationState.currentConversationMessageList.filter(
                            (msg) => messageIds.indexOf(msg.messageId) < 0
                        );
                }
            }
        });
        ...
        wfc.eventEmitter.on(EventType.SendMessage, (message) => {
            ...
            if (
                message.messageContent instanceof LeaveChannelChatMessageContent
            ) {
                return;
            }

            this._reloadConversation(message.conversation, true, message);
            if (!this._isDisplayMessage(message)) {
                return;
            }
            if (
                !conversationState.currentConversationInfo ||
                !message.conversation.equal(
                    conversationState.currentConversationInfo.conversation
                )
            ) {
            ...
            if (
                conversationState.currentConversationMessageList.length >
                defaultRenderMessageCount
            ) {
                conversationState.currentConversationMessageList =
                    conversationState.currentConversationMessageList.slice(
                        conversationState.currentConversationMessageList
                            .length - defaultRenderMessageCount
                    );
            }
        });
        ...
        wfc.eventEmitter.on(EventType.MessageStatusUpdate, (message) => {
            ...
            if (
                conversationState.currentConversationInfo.lastMessage &&
                conversationState.currentConversationInfo.lastMessage
        ...
        wfc.eventEmitter.on(EventType.MessageReceived, (delivery) => {
            if (conversationState.currentConversationInfo) {
                conversationState.currentConversationDeliveries =
                    wfc.getConversationDelivery(
                        conversationState.currentConversationInfo.conversation
                    );
            }
        });
        ...
        wfc.eventEmitter.on(EventType.MessageRead, (readEntries) => {
            ...
            if (conversationState.currentConversationInfo) {
                ...
                conversationState.currentConversationRead = new Map(
                    wfc.getConversationRead(
                        conversationState.currentConversationInfo.conversation
                    )
                );
            }
        ...
        if (isElectron()) {
            ...
            ipcRenderer.on("file-downloaded", (event, args) => {
                ...
                conversationState.downloadingMessages =
                    conversationState.downloadingMessages.filter(
                        (v) => !eq(v.messageUid, messageUid)
                    );
                let msg = wfc.getMessageByUid(messageUid);
                if (msg) {
                    msg.messageContent.localPath = localPath;
                    wfc.updateMessageContent(msg.messageId, msg.messageContent);
                    conversationState.currentConversationMessageList.forEach(
                        (m) => {
            ...
            ipcRenderer.on("file-download-progress", (event, args) => {
                ...
                let dm = conversationState.downloadingMessages.find((dm) =>
                    eq(dm.messageUid, messageUid)
                );
                if (dm) {
                    dm.progress = receivedBytes;
                    dm.total = totalBytes;
                }
                // console.log('file download progress', messageUid, receivedBytes, totalBytes);
            });

            if (
                !isMainWindow &&
                wfc.getConnectionStatus() ===
    ...
    _loadDefaultData() {
        ...
        conversationState.isMessageReceiptEnable =
            wfc.isReceiptEnabled() && wfc.isUserReceiptEnabled();
        // if (conversationState.currentConversationInfo) {
        //     this._loadCurrentConversationMessages();
        // }
    },
    ...
    _loadConversationList(conversationType = [0, 1, 3], lines = [0]) {
        ...
        conversationList.forEach((info) => {
            ...
            if (
                conversationState.currentConversationInfo &&
                conversationState.currentConversationInfo.conversation.equal(
                    info.conversation
                )
            ) {
    ...
    _reloadConversation(conversation, insertIfNoExist = true, msg) {
        let conversationInfo = wfc.getConversationInfo(conversation);
        if (conversationInfo) {
            conversationInfo = this._patchConversationInfo(
                conversationInfo,
                true
            );
        }
        let index = conversationState.conversationInfoList.findIndex((info) =>
            info.conversation.equal(conversation)
        );
        if (index >= 0) {
            Object.assign(
                conversationState.conversationInfoList[index],
                conversationInfo
            );
        } else {
            if (
                insertIfNoExist &&
                conversation.type !== ConversationType.ChatRoom &&
                !(
                    msg &&
                    msg.messageContent instanceof
                ...
                conversationState.conversationInfoList =
                    conversationState.conversationInfoList.reduce(
                        (current, next) => {
                            obj[next.target]
                                ? ""
                                : (obj[next.target] =
                                      true && current.push(next));
                            return current;
                        },
                        []
                    );
            ...
            if (cl) {
                ...
                cl.forEach((ci) => {
                    ...
                    if (conv.type === ConversationType.Single) {
                        ...
                        if (
                            lastMsg &&
                            uids.indexOf(lastMsg.from) >= 0 &&
                            toReloadConversations.findIndex(
                                (c) =>
                                    c.type === conv.type &&
                                    c.target === conv.target &&
                                    c.line === conv.line
                            ) === -1
                        ) {
                            toReloadConversations.push(conv);
    ...
    _reloadConversationByMessageIdIfExist(messageId) {
        ...
        for (
            let i = 0;
            i < conversationState.conversationInfoList.length;
            i++
        ) {
    ...
    _reloadConversationByMessageUidIfExist(messageUid) {
        ...
        for (
            ...
            if (
                info.lastMessage &&
                eq(info.lastMessage.messageUid, messageUid)
            ) {
                toLoadConversationInfo = info;
                break;
            }
    ...
    setCurrentConversation(conversation) {
        ...
        let convs = conversationState.conversationInfoList.filter((info) =>
            info.conversation.equal(conversation)
        );
        let info;
    ...
    setCurrentConversationInfo(conversationInfo) {
        if (!conversationInfo) {
            if (conversationState.currentConversationInfo) {
                let conversation =
                    conversationState.currentConversationInfo.conversation;
                wfc.unwatchOnlineState(conversation.type, [
                    conversation.target,
                ]);
        ...
        conversationState.currentConversationDeliveries =
            wfc.getConversationDelivery(conversationInfo.conversation);
        conversationState.currentConversationRead = wfc.getConversationRead(
            conversationInfo.conversation
        );
        ...
        if (conversation.type === ConversationType.Channel) {
            ...
            ConversationInfo && (ConversationInfo.anonymous_memberList = []);
        }

        //进入聊天框后check一下是否是好友
        ipcRenderer.send("send-check-friend", { type: conversation.type });
        // console.log(conversationInfo, '当前 currentConversationInfo')

        conversationState.quotedMessage = null;
        conversationState.currentConversationInfo._quotedMessage = null;
    ...
    subscribeChannel(channelId, subscribe, callback) {
        wfc.listenChannel(
            channelId,
            subscribe,
            () => {
    ...
    toggleMessageMultiSelection(message) {
        conversationState.enableMessageMultiSelection =
            !conversationState.enableMessageMultiSelection;
        pickState.messages.length = 0;
    ...
    selectOrDeselectMessage(message) {
        let index = pickState.messages.findIndex(
            (m) => m.messageId === message.messageId
        );
        if (index >= 0) {
    ...
    deleteSelectedMessages(type) {
        ...
        pickState.messages.forEach((m) => {
            type
                ? wfc.deleteRemoteMessageByUid(m.messageUid, null, null)
        ...
        targetConversations.forEach((conversation) => {
            ...
            if (
                ...
                messages.forEach((message) => {
                    // 针对转发匿名群发送媒体文件处理
                    if (conversation._extra) {
                        message.messageContent.extra = conversation._extra;
                        message.messageContent.pushData = conversation._extra;
                    }
                    // 公众里面的文章转发转换成url分享
                    if (
                        message.messageContent instanceof ArticlesMessageContent
                    ) {
                        let { title, url, digest, cover } =
                            message.messageContent.topArticle;
                        let messageContent =
                            Message.messageContentFromMessagePayload(
                ...
                let str3 = isEn ? "Group chat history" : "群的聊天记录";
                if (msgConversation.type === ConversationType.Single) {
                    let str1 = isEn ? " chat records" : "的聊天记录";
                    let str2 = isEn ? "and" : "和";
                    ...
                    title =
                        users[0]._displayName +
                        str2 +
                        users[1]._displayName +
                        str1;
                } else {
                    title = str3;
                }
            ...
            if (extraMessageText) {
                ...
                // 针对转发匿名群文字处理
                if (conversation._extra) {
                    textMessage.extra = conversation._extra;
                    textMessage.pushData = conversation._extra;
                }
    ...
    //  多选人员的时候， 新创建聊天发送
    newForwardByCreateConversation(
        forwardType,
        users,
        messages,
        extraMessageText
    ) {
        this.createSingleConversation(
            users,
            (conversation) => {
                this.forwardMessage(
                    forwardType,
    ...
    createSingleConversation(users, successCB, failCB) {
    ...
    // 转发中处理匿名信息
    async forwardAnonymousGroupMessage(params, filterAnonymous) {
        // console.log(params, filterAnonymous, "转发");
        if (!params) return;
        let { conversations, extraMessageText, forwardType, messages } = params;
        if (filterAnonymous) {
            let groupIds = conversations
                .filter((item) => {
                    return item.type === ConversationType.Group;
                })
                .map((item) => item.target);
            let anonymousGroupInfo = [];
            if (groupIds.length) {
                try {
                    ...
                    if (code === "000000") {
                        anonymousGroupInfo = data;
                    } else {
                        conversations = conversations.filter((item) => {
                            return !groupIds.includes(item.target);
                        });
                    }
                } catch (error) {
                    console.log(error);
            ...
            if (anonymousGroupInfo.length) {
                conversations.forEach((item) => {
                    if (item.type === ConversationType.Group) {
                        for (let j = 0; j < anonymousGroupInfo.length; j++) {
                            let anonymousInfo = anonymousGroupInfo[j];
                            if (anonymousInfo.groupId === item.target) {
                                let obj = {
                                    anonymous_name: anonymousInfo.anonymousName,
                                    portrait: anonymousInfo.anonymousPortrait,
                                    anonymous_en_name:
                                        anonymousInfo.anonymousEnName,
                                };
                                item._extra = JSON.stringify(obj);
                            }
                        }
                    }
                });
    ...
    previewMedia(src, thumbUrl, thumb, autoplay = true) {
        ...
        console.log(
            ...
            conversationState.previewMediaIndex
        );
    },
    ...
    previewMessage(message, continuous) {
        ...
        if (
            ...
            let mediaMsgs =
                conversationState.currentConversationMessageList.filter(
                    (m) =>
                        [MessageContentType.Image].indexOf(
                            m.messageContent.type
                        ) > -1
                );
            let msg;
            for (let i = 0; i < mediaMsgs.length; i++) {
                ...
                conversationState.previewMediaItems.push({
                    src: mediaUrl,
                    thumb:
                        "data:image/png;base64," + msg.messageContent.thumbnail,
                    autoplay: true,
                });
            }
        } else {
            ...
            message.messageContent.remotePath &&
                (message.messageContent.remotePath = this._checkRemotePath(
                    message.messageContent.remotePath
                ));
            ...
            conversationState.previewMediaItems.push({
                src: mediaUrl,
                thumb:
                    "data:image/png;base64," + message.messageContent.thumbnail,
                autoplay: true,
            });
        }
    },
    previewCompositeMessage(compositeMessage, focusMessageUid) {
        ...
        let mediaMsgs = compositeMessage.messageContent.messages.filter(
            (m) =>
     ...
     * @param {EXTRA} EXTRA 匿名群信息
     * @return {Promise<boolean>}
     */
    async sendFile(conversation, file, EXTRA = null) {
        console.log(file);
        // 大于 200M 的文件限制发送
        if (file.size > 209715200) {
            return false;
        }
        // console.log("send file", file, file.size);
        ...
        if (typeof file === "string") {
            ...
            let size = await this.getUrlImageInfo(file); // 获取图片内存大小
            // window 路径兼容
            let fileName = "";
            if (process.platform === "darwin") {
                fileName = file.substring(file.lastIndexOf("/") + 1);
            } else {
                let name = file.replace(new RegExp("\\\\", "g"), "=");
                if (name) {
                    let arr = name.split("=");
                    fileName = arr && arr.length && arr[arr.length - 1];
                }
            }
            file = {
                path: file,
            ...
            fileOrLocalPath = file; // file || Object 类型
        }
        let msg = new Message();
        msg.conversation = conversation;

        let mediaType = helper.getMediaType(
        ...
        switch (messageContentmediaType) {
            ...
            case MessageContentMediaType.File:
                messageContent = new FileMessageContent(
                    fileOrLocalPath,
                    remotePath
                );
                break;
        ...
        if (EXTRA) {
            ...
            // console.log(messageContent.extra, EXTRA, '匿名群处理');
            if (messageContent.extra) {
                let { anonymous_name, anonymous_en_name } = EXTRA;
                let portraitUrl = `${Config.STATIC_SERVER}/im-fs${EXTRA.portrait}`;
                messageContent.extra.anonymous_name = anonymous_name;
                messageContent.extra.portrait = portraitUrl;
                messageContent.extra.anonymous_en_name = anonymous_en_name;
                messageContent.pushData.anonymous_name = anonymous_name;
                messageContent.pushData.portrait = portraitUrl;
                messageContent.pushData.anonymous_en_name = anonymous_en_name;
            } else {
                messageContent.extra = JSON.stringify(EXTRA);
                messageContent.pushData = JSON.stringify(EXTRA);
            }
        }
        let imFileId = generateUUID();
        let imFileName = file.name;
        messageContent.imFileId = imFileId;
        messageContent.imFileName = imFileName;
        messageContent.imFileSize = file.size;
        messageContent.fromDiskShare = "";
        msg.messageContent = messageContent;
        wfc.sendMessage(
            msg,
            (messageId) => {
                msg.messageId = messageId;
                console.log("sf, pr", msg, messageId);
            },
            (progress, total) => {
                // console.log('sf p', msg.messageId, Math.ceil(progress / total * 100))
                let sm = conversationState.sendingMessages.find(
            ...
            (messageUid) => {
                ...
                conversationState.sendingMessages =
                    conversationState.sendingMessages.filter(
                        (e) => e.messageId !== msg.messageId
                    );
     ...
     *
     * @param conversation
     * @param {Object} info 链上存储文档 当前文件信息
     * @param {EXTRA} EXTRA 匿名群信息
     * @return {Promise<boolean>}
     */
    async sendCloudFile(conversation, info, EXTRA = null) {
        let msg = await this.cloudMessageContent(info, EXTRA);
        if (!msg) return;
        msg.conversation = conversation;
        wfc.sendMessage(
            msg,
    ...
    // 处理云盘文件消息体
    async cloudMessageContent(info, EXTRA = null) {
        let msg = new Message();
        let mediaType,
        ...
        if (
            ...
            ].includes(messageContentmediaType) &&
            shortcutUrl &&
            shortcutUrl !== `${Config.STATIC_SERVER}/shotcut/null`
        ) {
            let base64Image = await this.getUrlBase64(shortcutUrl);
            iThumbnail = base64Image && base64Image.split(",")[1];
            if (!iThumbnail) return false;
        }
        ...
        if (
            ...
            try {
                let res = await getUpload2Minio({
                    imFileId,
                    originFileId: fileId,
                    fileType: messageContentmediaType,
                });
                let { code, data } = res.data;
                if (code === "000000") {
                    remotePath = `${Config.STATIC_SERVER}/im-fs/${data}`;
                } else {
                    return false;
                }
            } catch (error) {
        ...
        switch (messageContentmediaType) {
            ...
            case MessageContentMediaType.Video:
                ...
                messageContent.duration =
                    videoTime && Math.ceil(Number(videoTime) * 1000);
                break;
            case MessageContentMediaType.File:
                messageContent = new CloudDiskMessageContent(
                    fileId,
                    fileName,
                    iconUrl,
                    shareId,
        ...
        if (EXTRA) {
            console.log(messageContent.extra, EXTRA, "匿名群处理");
            if (messageContent.extra) {
                let { anonymous_name, anonymous_en_name, portrait } = EXTRA;
                let portraitUrl = `${Config.STATIC_SERVER}/im-fs${portrait}`;
                messageContent.extra.anonymous_name = anonymous_name;
                messageContent.extra.portrait = portraitUrl;
                messageContent.extra.anonymous_en_name = anonymous_en_name;
                messageContent.pushData.anonymous_name = anonymous_name;
        ...
        // 针对发送链上存储文档 媒体文件处理（图片和视频）
        messageContent.fileId = fileId;
        messageContent.imFileId = imFileId;
        messageContent.shareId = shareId;
        messageContent.iconUrl = iconUrl;

        messageContent.fileName = fileName;
        messageContent.name = fileName;
        messageContent.imFileName = fileName;

        messageContent.size = fileSize;
        messageContent.fileSize = fileSize;
        messageContent.imFileSize = fileSize;
        ...
        return new Promise((resolve, reject) => {
            ...
            img.onload = function () {
                ...
                let ext = img.src
                    ...
                    .toLowerCase();
                const dataURL = canvas.toDataURL("image/" + ext); //Base64编码
                // console.log(dataURL, '缩略图');
                resolve(dataURL);
                canvas = null;
            };
            ...
            // 放在onload后，防止图片有缓存加载过快，来不及给onload赋值
            img.src = imgUrl;
        });
    },
    // 通过url 去获取图片的内存大小
    getUrlImageInfo(url) {
        return new Promise((resolve) => {
            let img = new Image();
            img.src =
                url.startsWith("http") > -1 ? url : "local-resource://" + url;
            img.onload = function () {
    ...
    quoteMessage(message) {
        ...
        // conversationState.currentConversationInfo._quotedMessage = message; // 有副作用，暂时隐藏
    },
    ...
    _loadCurrentConversationMessages() {
        ...
        wfc.getMessagesV2(
            ...
            (msgs) => {
                ...
                this._patchCurrentConversationMessages();
                if (msgs.length) {
                    conversationState.currentConversationOldestMessageId =
                        msgs[0].messageId;
                }
    ...
    _onloadConversationMessages(conversation, messages) {
        ...
        conversationState.currentConversationMessageList = newMsgs.concat(
            conversationState.currentConversationMessageList
        );
        return loadNewMsg;
    },
    loadConversationHistoryMessages(loadedCB, completeCB) {
        ...
        let conversation =
            conversationState.currentConversationInfo.conversation;
        console.log(
            "loadConversationHistoryMessage",
            conversation,
            conversationState.currentConversationOldestMessageId,
        ...
        let loadRemoteHistoryMessageFunc = () => {
            wfc.loadRemoteConversationMessages(
                ...
                (msgs, hasMore) => {
                    console.log(
                        "loadRemoteConversationMessages response",
                        msgs.length
                    );
                    if (msgs.length === 0) {
                        // 拉回来的消息，本地全都有时，会走到这儿
                        ...
                        if (
                            ...
                            this._onloadConversationMessages(
                                conversation,
                                msgs
                            );
                        }
    ...
    removeConversation(conversation) {
        ...
        conversationState.conversationInfoList =
            conversationState.conversationInfoList.filter(
                (info) => !info.conversation.equal(conversation)
            );
        this.updateTray();
    },

    // 双侧删除会话
    async removeDoubleConversation(msg) {
        // 清除会话内容
        await wfc.clearRemoteConversationMessages(msg.conversation);
        // 删除自己方
        this.removeConversation(msg.conversation);
    },
    ...
    _patchMessage(m, lastTimestamp = 0, userInfoMap) {
        ...
        if (m.conversation.type === ConversationType.Single) {
            m._from = userInfoMap
                ? userInfoMap.get(m.from)
                : wfc.getUserInfo(m.from, false, "");
        }
        if (!m._from || !m._from.updateDt) {
            let u = wfc.getUserInfo(
                m.from,
                false,
                m.conversation.type === ConversationType.Group
        ...
        if (
            numberValue(lastTimestamp) > 0 &&
            numberValue(m.timestamp) - numberValue(lastTimestamp) >
                5 * 60 * 1000
        ) {
            m._showTime = true;
        }
        ...
        // 图片路径不全问题
        if (m.messageContent.remotePath) {
            m.messageContent.remotePath = this._checkRemotePath(
                m.messageContent.remotePath
            );
        }
    ...
    _patchConversationInfo(
        info,
        patchLastMessage = true,
        userInfoMap,
        groupInfoMap
    ) {
        if (info.conversation.type === ConversationType.Single) {
            info.conversation._target = userInfoMap
                ? userInfoMap.get(info.conversation.target)
                : wfc.getUserInfo(info.conversation.target, true);
            if (info.conversation._target) {
                info.conversation._target._displayName =
                    wfc.getUserDisplayNameEx(info.conversation._target);
            }
        } else if (info.conversation.type === ConversationType.Group) {
            info.conversation._target = groupInfoMap
                ? groupInfoMap.get(info.conversation.target)
                : wfc.getGroupInfo(info.conversation.target, true);
            if (info.conversation._target) {
                info.conversation._target._isFav = wfc.isFavGroup(
                    info.conversation.target
                );
                info.conversation._target._displayName = info.conversation
                    ._target.remark
                    ? info.conversation._target.remark
                    : info.conversation._target.name;
            }
        } else if (info.conversation.type === ConversationType.Channel) {
            info.conversation._target = wfc.getChannelInfo(
                info.conversation.target,
                false
            );
            info.conversation._target._displayName =
        ...
        if (
            patchLastMessage &&
            info.lastMessage &&
            info.lastMessage.conversation !== undefined &&
            (!info.lastMessage._from || !info.lastMessage._from.updateDt)
        ) {
    ...
    isDownloadingMessage(messageUid) {
        ...
        return (
            conversationState.downloadingMessages.findIndex((dm) =>
                eq(dm.messageUid, messageUid)
            ) >= 0
        );
    },
    ...
    getDownloadingMessageStatus(messageUid) {
        return conversationState.downloadingMessages.find((dm) =>
            eq(dm.messageUid, messageUid)
        );
    },

    getSendingStatus(messageId) {
        return conversationState.sendingMessages.find(
    ...
    removeFloatingConversation(conversation) {
        conversationState.floatingConversations =
            conversationState.floatingConversations.filter(
                (c) => !c.equal(conversation)
            );
    },

    isConversationInCurrentWindow(conversation) {
        if (miscState.isMainWindow) {
            let index = conversationState.floatingConversations.findIndex(
                (fc) => fc.equal(conversation)
            );
            return index === -1;
        } else {
            return (
    ...
    _loadSelfUserInfo() {
        ...
        // 名称为空的时候默认设置密保手机号为名称
        if (!contactState.selfUserInfo.displayName) {
            let mobile = contactState.selfUserInfo.mobile;
            contactState.selfUserInfo.displayName = mobile.replace(
                /(\d{3})\d*(\d{4})/,
                "$1****$2"
            );
        }
    ...
    _loadFriendList() {
        console.log("store _loadFriendList");
    ...
    _patchCurrentConversationOnlineStatus() {
        ...
        if (
            convInfo &&
            convInfo.conversation.type === ConversationType.Single
        ) {
            ...
            conversationState.currentConversationInfo.conversation._targetOnlineStateDesc =
                this.getUserOnlineState(convInfo.conversation.target);
        } else {
            //TODO
        }
    },
    _loadFriendRequest() {
        ...
        requests.sort(
            (a, b) => numberValue(b.timestamp) - numberValue(a.timestamp)
        );
        ...
        contactState.unreadFriendRequestCount =
            wfc.getUnreadFriendRequestCount();
    },
    ...
    _patchAndSortUserInfos(userInfos, groupId = "", compareFn) {
        userInfos = userInfos.map((u) => {
            ...
            u._pinyin = convert(u._displayName, { style: 0 })
                .join("")
                .trim()
                .toLowerCase();
            ...
            u._firstLetters = convert(u._displayName, { style: 4 })
                .join("")
                .trim()
                .toLowerCase();
            return u;
        ...
        if (compareFn) {
            ...
            userInfos = userInfos.sort((a, b) =>
                a.__sortPinyin.localeCompare(b.__sortPinyin)
            );
        }
    ...
    _loadChannelList() {
        wfc.getRemoteListenedChannels(
            (channelIds) => {
                if (channelIds) {
                    contactState.channelList = channelIds.map((channelId) =>
                        wfc.getChannelInfo(channelId, false)
                    );
                    contactState.channelList = contactState.channelList.filter(
                        (ch) => {
                            return !(ch instanceof NullChannelInfo);
                        }
                    );
                    // contactState.channelList = this._patchAndSortChannels(contactState.channelList)
                }
            },
            (err) => {
    ...
    _patchAndSortChannels(channels) {
        channels = channels.map((u) => {
            u._pinyin = convert(u.name, { style: 0 })
                .join("")
                .trim()
                .toLowerCase();
            let firstLetter = u._pinyin[0];
            ...
            u._firstLetters = convert(u.name, { style: 4 })
                .join("")
                .trim()
                .toLowerCase();
            return u;
    ...
    setFindNewFriend() {
        ...
        contactState.findNewFriend = true;
    },
    setCurrentFriendRequest(friendRequest) {
        ...
        contactState.currentGroup = null;
        contactState.currentChannel = null;
        contactState.findNewFriend = false;
    },
    ...
    toggleFriendRequestList() {
        contactState.expandFriendRequestList =
            !contactState.expandFriendRequestList;
    },
    ...
    setSearchQuery(query, from) {
        ...
        if (query) {
            ...
            searchState.conversationSearchResult =
                this.filterConversation(query);
            // 123456# 秘密令牌
            searchState.secretSearchResult = this.filterSecretInfo(query);
            // 默认搜索未关注+已关注
            // this.searchChannel(query, from);
        ...
        wfc.searchUser(
            ...
            (keyword, userInfos) => {
                ...
                if (searchState.query === keyword) {
                    searchState.userSearchResult = userInfos.filter(
                        (u) => !wfc.isMyFriend(u.uid)
                    );
                }
            },
            (err) => {
    ...
    searchChannel(query, from) {
        if (from) {
            console.log("搜索全部类型-已关注+未关注");
            wfc.searchChannel(
                query,
                true,
                (list) => {
                    searchState.channelSearchResult = list;
                },
                (error) => {
                    console.log(error);
            ...
            console.log("搜索-已关注");
            if (contactState.channelList.length) {
                searchState.channelSearchResult =
                    contactState.channelList.filter((u) => {
                        return (
                            u.name.indexOf(query) > -1 ||
                            u._firstLetters.indexOf(query.toLowerCase()) > -1 ||
                            u._pinyin.indexOf(query.toLowerCase()) > -1
                        );
                    });
            } else {
                searchState.channelSearchResult = [];
    ...
    //服务器搜索用户（新增）
    searchSeverUser(query, callback) {
        console.log("search user", query);
        // 精准搜索手机号
        wfc.searchUser(
            query,
            ...
            0,
            (keyword, userInfos) => {
                console.log("search user result", keyword, query, userInfos);
    ...
    filterContact(query) {
        let result = contactState.friendList.filter((u) => {
            return (
                u._displayName.indexOf(query) > -1 ||
                u._firstLetters.indexOf(query.toLowerCase()) > -1 ||
                u._pinyin.indexOf(query.toLowerCase()) > -1
            );
        });
    ...
    filterUsers(users, filter) {
        ...
        let queryPinyin = convert(filter, { style: 0 })
            .join("")
            .trim()
            .toLowerCase();
        let result = users.filter((u) => {
            if (u.anonymousName || u.anonymousEnName) {
                return (
                    u.anonymousName.indexOf(filter) > -1 ||
                    u.anonymousEnName.indexOf(filter) > -1
                );
            }
            if (u._displayName || u._pinyin || u._firstLetters) {
                return (
                    u._displayName.indexOf(filter) > -1 ||
                    u._displayName.indexOf(queryPinyin) > -1 ||
                    u._pinyin.indexOf(filter) > -1 ||
                    u._pinyin.indexOf(queryPinyin) > -1 ||
                    u._firstLetters.indexOf(filter) > -1 ||
                    u._firstLetters.indexOf(queryPinyin) > -1
                );
            }
        });
        console.log(queryPinyin, "=dddd==", result);
        return result;
    },

    // 会议中搜索成员
    filterConferenceMember(users, filter) {
        if (!users || !filter || !filter.trim()) {
            return users;
        }
        let queryPinyin = convert(filter, { style: 0 })
        ...
        let result = users.filter((u) => {
            if (u.displayName) {
                return (
                    u.displayName.indexOf(filter) > -1 ||
                    u.displayName.indexOf(queryPinyin) > -1
                );
            } else if (u._displayName) {
                return (
                    u._displayName.indexOf(filter) > -1 ||
                    u._displayName.indexOf(queryPinyin) > -1
                );
    ...
    filterFavGroup(query) {
        ...
        let queryPinyin = convert(query, { style: 0 })
            .join("")
            .trim()
            .toLowerCase();
        let result = contactState.favGroupList.filter((g) => {
            let groupNamePinyin = convert(g.name, { style: 0 })
                .join("")
                .trim()
                .toLowerCase();
            return (
                g.name.indexOf(query) > -1 ||
                g.name.indexOf(queryPinyin) > -1 ||
                groupNamePinyin.indexOf(query) > -1 ||
                groupNamePinyin.indexOf(queryPinyin) > -1
            );
        });
    ...
    filterConversation(query) {
        return conversationState.conversationInfoList.filter((info) => {
            let displayNamePinyin = convert(
                ...
                .trim()
                .toLowerCase();
            let firstLetters = convert(info.conversation._target._displayName, {
                style: 4,
            })
                .join("")
                .trim()
                .toLowerCase();
            return (
                info.conversation._target._displayName.indexOf(query) > -1 ||
                displayNamePinyin.indexOf(query.toLowerCase()) > -1 ||
                firstLetters.indexOf(query) > -1
            );
        });
    },
    //TODO 秘密令牌 12345#初始秘密
    filterSecretInfo(query) {
        let secretTokenStore = useSecretTokenStore();
        const { secretPassword, secretStatus, secretConfig } =
            storeToRefs(secretTokenStore);
        // 符合条件 开启秘密好友和群 有密码，且正确输入
        let reg = /.*#$/;
        if (
            reg.test(query) &&
            Md5(query.substring(query.length - 1, 0)).toString() ===
                secretPassword.value &&
            secretStatus.value &&
            secretConfig.value
        ) {
            this.toggleSecretTokenConversation(0);
            this.toggleSearchView(false);
            return;
        }
        console.log(secretConfig.value, secretPassword.value);
        // 显示默认流程
        if (
            !secretConfig.value &&
            !secretPassword.value &&
            ...
            secretTokenStore.setSecretSearch(true);
            return;
        }
        // 忘记密码
        if (query === "654321#") {
            secretTokenStore.setForgotPassword(true);
            return;
        }
    },
    toggleSecretSearchView(toggle) {
        let secretTokenStore = useSecretTokenStore();
        secretTokenStore.setSecretSearch(toggle);
    },
    toggleSetSecretTokenView(toggle) {
        let secretTokenStore = useSecretTokenStore();
        secretTokenStore.setSecretToken(toggle);
    },
    toggleSetForgotPasswordView(toggle) {
        let secretTokenStore = useSecretTokenStore();
        secretTokenStore.setForgotPassword(toggle);
    },
    // 切换显示、隐藏好友，群聊 toggle : 0 / 1
    toggleSecretTokenConversation(toggle) {
        let secretTokenStore = useSecretTokenStore();
        secretTokenStore.changeSecretStatusAsync({
            safeFriendHideStatus: toggle,
        });
    },
    // 登录成功之后获取数据
    initSecretTokenConfig() {
        let secretToken = useSecretTokenStore();
        // 重置默认数据
        secretToken.resetSecretTokenAllData();
        secretToken.secretTokenConfigAsync();
        secretToken.secretFriendsOrGroupsAsync();
    },
    // 通过uid 获取该用户的conversation 信息 指定会议用
    getDesignatedMemberConversations(uids) {
        let conversations1 = [];
        let conversations2 = [];
        let arr1 = [];
        let arr2 = [];
        uids.forEach((uid) => {
            conversationState.conversationInfoList.forEach((info) => {
                if (info.conversation._target.uid === uid) {
                    arr1.push(uid);
                    conversations1.push(info.conversation);
                }
            });
        });
        if (arr1.length < uids.length) {
            arr2 = uids.filter((u) => arr1.findIndex((iu) => iu === u) === -1);
            if (arr2.length) {
                arr2.forEach((uid) => {
                    let conversation = new Conversation(
                        ConversationType.Single,
                    ...
                    conversation._target = wfc.getUserInfo(
                        conversation.target,
                        false
                    );
                    conversation._target._displayName =
                        wfc.getUserDisplayNameEx(conversation._target);
    ...
    // 通过群组id 获取群的conversation 信息 指定会议用
    getDesignatedGroupConversations(groupId) {
        let conversation = conversationState.conversationInfoList.filter(
            (item) => {
                return item.target === groupId;
            }
        )[0].conversation;
        if (!conversation) {
    ...
    filterGroupConversation(query) {
        ...
        // })
        let groups = conversationState.conversationInfoList
            .filter((info) => info.conversation.type === ConversationType.Group)
            .map((info) => info.conversation._target.target);
        let gsr = wfc.searchGroups(query);
        // 过滤下本地群组
        return gsr
            .filter((r) => {
                return groups.includes(r.groupInfo.target);
            })
            .map((r) => r.groupInfo);
    ...
    searchMessageInTypes(conversation, contentTypes, query, offset) {
        let msgs = wfc.searchMessageByTypes(
            conversation,
            query,
            contentTypes,
            true,
    ...
    searchConversation(query, types = [0, 1], lines = [0, 1]) {
    ...
    pickOrUnpickConversation(conversation) {
        let index = pickState.conversations.findIndex(
            (c) =>
                conversation.target === c.target &&
                conversation.line === c.line &&
                conversation.type === c.type
        );
        if (index >= 0) {
            pickState.conversations = pickState.conversations.filter(
                (c) =>
                    !(
                        conversation.target === c.target &&
                        conversation.line === c.line &&
    ...
    handleUserName(userInfo) {
        if (userInfo) {
            let { displayName, _displayName, mobile } = userInfo;
            let name =
                displayName ||
                _displayName ||
                mobile ||
                (isEn ? "Unknown user" : "未知用户");
            if (name.length >= 34 && /^<(?![^a-zA-Z]+$)(?!\D+$)/.test(name)) {
                return this.resetPhoneNumber(mobile);
            }
            if (/^1[3|4|5|7|8|9][0-9]{9}$/.test(name)) {
                return this.resetPhoneNumber(name);
            }
            return name;
        } else {
            return isEn ? "Unknown user" : "未知用户";
        }
    },

    // 设置脱敏手机号
    resetPhoneNumber(tel) {
     ...
     * 创建会话、创建群组
     * params :
     *  1、usersInfo {Object} { users {Array} true  minGroupMember {Number} false }
     * 2、successCB {function}
     * 3、failCB {function}
     *  */
    async createConversation(usersInfo, successCB, failCB) {
        let { users, minGroupMember, groupName } = usersInfo;
        // groupName
        if (users.length === 1 && !minGroupMember) {
            let conversation = new Conversation(
                ConversationType.Single,
                users[0].uid,
                0
        ...
        // 创建群聊
        // 1、拼接群聊名称
        let defaultGroupName = this.handleUserName(contactState.selfUserInfo);
        let groupMemberIds = [];
        // let groupMemberPortraits = [contactState.selfUserInfo.portrait];
        for (let i = 0; i < users.length; i++) {
            groupMemberIds.push(users[i].uid);
            if (i <= 3) {
                defaultGroupName += "、" + this.handleUserName(users[i]);
            }
        }
        defaultGroupName = defaultGroupName.substr(0, 60);
        let createGroupMemberIds = [
            ...new Set([wfc.getUserId(), ...groupMemberIds]),
        ];
        // 3、创建群
        wfc.createGroup(
            null,
            ...
            null,
            (groupId) => {
                this.updateGroupPortraitFromAjax(groupId, createGroupMemberIds);
                let conversation = new Conversation(
                    ConversationType.Group,
                    groupId,
                    0
    ...
    _loadUserLocalSettings() {
        ...
        miscState.enableNotificationMessageDetail =
            setting === null || setting === "1";
        miscState.enableCloseWindowToExit =
            getItem(userId + "-" + "closeWindowToExit") === "1";
    ...
    setEnableNotification(enable) {
        ...
        setItem(
            contactState.selfUserInfo.uid + "-" + "notification",
            enable ? "1" : "0"
        );
    },
    ...
    setEnableNotificationDetail(enable) {
        ...
        setItem(
            contactState.selfUserInfo.uid + "-" + "notificationDetail",
            enable ? "1" : "0"
        );
    },
    ...
    setEnableCloseWindowToExit(enable) {
        ...
        setItem(
            contactState.selfUserInfo.uid + "-" + "closeWindowToExit",
            enable ? "1" : "0"
        );
    ...
    setEnableAutoLogin(enable) {
        ...
        setItem(
            contactState.selfUserInfo.uid + "-" + "autoLogin",
            enable ? "1" : "0"
        );
    },
    getEnableAutoLogin(userId) {
        return getItem(userId + "-" + "autoLogin") === "1";
    },
    ...
    getGroupMemberUserInfos(groupId, includeSelf = true, sortByPinyin = false) {
        ...
        if (sortByPinyin) {
            ...
            return this._patchAndSortUserInfos(
                userInfosCloneCopy,
                groupId,
                compareFn
            );
    ...
    getConversationFileRecords(
        ...
        wfc.getConversationFileRecords(
            conversation,
            fromUser,
            beforeMessageUid,
            0,
    ...
    deleteFriend(target) {
        wfc.deleteFriend(
            ...
            () => {
                ...
                conversationState.conversationInfoList =
                    conversationState.conversationInfoList.filter(
                        (info) => !info.conversation.equal(conv)
                    );
            },
            (err) => {
    ...
    _patchFileRecords(fileRecords) {
        fileRecords.forEach((fileRecord) => {
            let groupId =
                fileRecord.conversation.type === 1
                    ? fileRecord.conversation.target
                    : "";
            if (groupId) {
                fileRecord._userDisplayName = wfc.getGroupMemberDisplayName(
                    groupId,
                    fileRecord.userId
                );
            } else {
                fileRecord._userDisplayName = wfc.getUserDisplayName(
                    fileRecord.userId
                );
            }
            ...
            if (fileRecord.conversation.type === 0) {
                fileRecord._conversationDisplayName = isEn
                    ? `Chat with ${conversationInfo.conversation._target._displayName}`
                    : `与${conversationInfo.conversation._target._displayName}的聊天`;
            } else {
                fileRecord._conversationDisplayName =
                    conversationInfo.conversation._target._displayName;
            }
            if (
                fileRecord.name.indexOf(FileMessageContent.FILE_NAME_PREFIX) ===
                0
            ) {
                fileRecord.name = fileRecord.name.substring(
                    fileRecord.name.indexOf(
                        FileMessageContent.FILE_NAME_PREFIX
            ...
            fileRecord._fileIconName = helper.getFiletypeIcon(
                fileRecord.name.substring(fileRecord.name.lastIndexOf("."))
            );
        });
    },
    ...
    notify(msg) {
        ...
        if (
            ...
            now - this._lastNotificationTime < 4000
        ) {
            return;
        }
        ...
        //Todo
        if (
            msg.direction === 0 /* && !(type===0 && target===file_transfer_id)*/
        ) {
            return;
        }
        // console.log(content.type, MessageConfig.getMessageContentPersitFlag(content.type), '我的notify===')
        if (
            ...
            if (
                ...
                tip = isEn ? "New message is coming" : "新消息来了";
            } else {
                tip = isEn ? "Someone@you" : "有人@你";
            }
            // 不支持该消息类型，请在手机上查看 ===》》  不提示
            //extra 中埋此字段 \"chatMode\":\"1\" 阅后即焚 0 正常
            try {
                if (content.extra && typeof content.extra === "string") {
                    let extraObj = JSON.parse(content.extra);
            ...
            // console.log(tip, '我notify提示title==》》》', miscState.enableNotificationMessageDetail, '消息详情==》》', content.digest(msg))
            Push.create(tip, {
                body: miscState.enableNotificationMessageDetail
                    ? content.digest(msg)
                    : "",
                ...
                onClick: () => {
                    if (isElectron()) {
                        ipcRenderer.send(
                            IPCEventType.CLICK_NOTIFICATION,
                            currentWindow.getMediaSourceId()
                        );
                    } else {
                        window.focus();
                        this.close();
    ...
    updateLinuxTitle(unreadCount) {
        this.updateLinuxTitle.title = isEn ? "ChainPal" : "链上会";
        ...
        if (!miscState.linuxUpdateTitleInterval) {
            miscState.linuxUpdateTitleInterval = setInterval(() => {
                if (
                    this.updateLinuxTitle.showTitle ||
                    this.updateLinuxTitle.unreadCount < 1
                ) {
                    document.title = this.updateLinuxTitle.title;
                } else {
                    document.title =
                        this.updateLinuxTitle.title +
                        " " +
                        this.updateLinuxTitle.unreadCount;
                }
                this.updateLinuxTitle.showTitle =
                    !this.updateLinuxTitle.showTitle;
            }, 1000);
    ...
    // 云盘资源中 移动端部分图片视频路径 是 /fs/xxxxx,或者 fs/xxxx 类型的路径需要处理一下
    _checkRemotePath(path) {
        // 云盘有不完整路径拼接完整后保存接口
    ...
    setCurrentHistoryConference(conferenceInfo) {
        conferenceState.currentHistoryConference = conferenceInfo;
    },
    //通过接口请求头像生成
    async updateGroupPortraitFromAjax(groupId, userIds) {
        try {
            const res = await generateGroupPortrait({
                groupId,
                userIds,
                userId: wfc.getUserId(),
            });
            ...
            console.log(
                data,
                message,
                code === "000000" ? "生成头像成功" : "生成头像失败"
            );
        } catch (error) {}
    },
    _getLocalGroupList(query) {
        let groups = conversationState.conversationInfoList
            .filter((item) => item.conversationType === 1)
            .map((item) => {
                let { name, owner, portrait, target } =
                    item.conversation._target;
                return {
                    groupId: target,
                    groupName: name,
                    ownerId: owner,
        ...
        if (query) {
            return groups.filter((item) => item.groupName.indexOf(query) > -1);
        } else {
            return groups;
        }
    },
    onCustomizeMessage(msg) {
        ...
        if (
            [
                MessageContentType.Account_Cancellation_Notification,
                MessageContentType.Account_Suspension_Notification,
            ...
            msg.content.type ===
                MessageContentType.Account_Cancellation_Notification &&
                ipcRenderer.send("account-cancellation");
            return;
        }
        ...
        if (
            msg.messageContent.type ===
                MessageContentType.Remove_Double_Conversation ||
            msg.messageContent.type ===
                MessageContentType.Remove_Double_Group_Conversation
        ) {
            this.removeDoubleConversation(msg);
        }
        // 监听会员变化
        if (msg.content.type === MessageContentType.Vip_Change) {
            ipcRenderer.send("vip-change-permission");
        }
        // 监听快速会议铃声 1034
        if (msg.content.type === MessageContentType.Voip_QuickMeeting_Ring) {
            // status: // 0 呼起 1：关闭
            let status = Number(msg.messageContent.status);
            if (status) {
                ipcRenderer.send(IPCEventType.Close_Meeting_Ring); // 关闭窗口
                return;
            }
            // status:0 ,且不是自呼，且当前没有在会议中
            if (
                !status &&
                msg.messageContent.host !== wfc.getUserId() &&
                !avenginekitproxy.callWin &&
                !avenginekitproxy.conference
            ) {
                let hash = window.location.hash;
                let url = window.location.origin;
                let path = `/voip/quickcall?ts=${+new Date()}`;
                url = hash
                    ? window.location.href.replace(hash, `#${path}`)
                    : (url += path);
                ipcRenderer.send(IPCEventType.Quick_Meeting_Ring, { msg, url });
                return;
            }
        }
        // 1036 暂时不处理 4.4.9 处理
        if (
            msg.content.type ===
            MessageContentType.AddressBook_ChatFriend_Notification
        ) {
            // console.log(msg.content.type, 'return 出去')
            return;
        }
        // 1038
        if (
            msg.content.type === MessageContentType.Secret_Friend_Notification
        ) {
            let secretTokenStore = useSecretTokenStore();
            msg.content.content
                ? secretTokenStore.secretTokenConfigAsync()
                : secretTokenStore.secretFriendsOrGroupsAsync(); // 更新一下接口
