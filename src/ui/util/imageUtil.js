...
async function getConversationPortrait(
    conversation,
    userInfoMap,
    groupInfoMap
) {
    ...
    switch (conversation.type) {
        case ConversationType.Single:
            let u = userInfoMap
                ? userInfoMap.get(conversation.target)
                : wfc.getUserInfo(conversation.target, true);
            portrait = u.portrait;
            break;
        case ConversationType.Group:
            portrait = await getGroupPortrait(
                conversation.target,
                groupInfoMap
            );
            break;
...
async function getGroupPortrait(groupId, groupInfoMap) {
    let groupInfo = groupInfoMap
        ? groupInfoMap.get(groupId)
        : wfc.getGroupInfo(groupId, false);
    ...
    if (!portrait || now - portrait.timestamp > 30 * 1000) {
        ...
        groupMembers = groupMembers.filter(
            m => m.type !== GroupMemberType.Removed
        );
...
function imageThumbnail(file) {
    return new Promise((resolve, reject) => {
        ...
        img.setAttribute("crossOrigin", "anonymous");
        img.onload = () => {
            let resizedCanvas = resizeImage.resize2Canvas(img, 200, 100);
