    ...
    getFiletypeIcon: (extension) => {
        ...
        switch (true) {
            ...
            case ['xls', 'xlsx'].includes(extension):
                filename = 'excel';
                break;
            case ['ppt', 'pptx', 'keynote'].includes(extension):
                filename = 'ppt';
                break;
            case ['txt'].includes(extension):
                filename = 'txt';
                break;
    ...
    weekFormat: (num) => {
        ...
        switch (num) {
            case 1:
                ...
                break
            case 2:
                str = isEn ? 'Tuesday' : '星期二'
                break
            case 3:
                str = isEn ? 'Wednesday' : '星期三'
                break
            case 4:
                str = isEn ? 'Thursday' : '星期四'
                break
            case 5:
                str = isEn ? 'Friday' : '星期五'
                break
            case 6:
                str = isEn ? 'Saturday' : '星期六'
                break
            default:
                str = isEn ? 'Sunday' : '星期天'
        }
    ...
    timeFormat: (date) => {
        ...
        if (Y < nowY) {
            ...
            if (Y === nowY && M === nowM && (nowD - D <= 7)) {
                ...
                if ((nowD - D) === 1) {
                    // yesterday
                    newtime = (isEn ? 'Yesterday' : '昨天') + ' ' + H + ':' + Min
                } else if (nowD === D) {
                    // 当天
                    newtime = H + ':' + Min
                } else {
