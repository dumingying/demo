  ...
  <section class="fav-list-container">
    ...
    <div infinite-wrapper v-on:scroll="onScroll">
      <div v-if="category === 'media'">
        <div
          v-for="group in groupedMediaItems"
          :key="group.category"
        ...
        >
          ...
          <ul class="media-category-items">
            <li
              v-for="(favItem, index) in group.items"
      ...
      <ul v-else>
        <li
          v-for="(favItem, index) in filteredFavItems"
          @contextmenu="openFavContextMenu($event, favItem)"
          :key="index"
        >
          <div class="fav-item-container" @click="handleClick(favItem)">
            <div class="fav-item-content">
              ...
              <div v-if="favItem.type === 5" class="fav-item-file">
                <img
                  :src="require(`@/assets/images/filetypes/${favItem._fileIconName}`)"
                  alt=""
                />
                <div class="name-size">
...
export default {
  ...
  data() {
    return {
      favItems: [],
      imagePlaceHolder:
        "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNcunDhfwAGwgLoe4t2fwAAAABJRU5ErkJggg==",
      infiniteId: +new Date(),
    };
  },
  methods: {
    ...
    async loadFavList(category, cb) {
      let startId =
        this.favItems.length > 0 ? this.favItems[this.favItems.length - 1].id : 0;
      ...
      if (data && data.result) {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          if (
            category === "media" &&
            [MessageContentType.Image, MessageContentType.Video].indexOf(items[i].type) >=
              0
          ) {
            found = true;
            break;
          ...
          if (
            category === "composite" &&
            items[i].type === MessageContentType.Composite_Message
          ) {
            found = true;
            break;
          }
    ...
    _patchFavItem(favItems) {
      favItems.forEach((fi) => {
          ...
          if (fi.type === MessageContentType.Composite_Message) {
            ...
            if (compositeContent instanceof CompositeMessageContent) {
              for (let i = 0; i < compositeContent.messages.length && i < 2; i++) {
                ...
                fi._content += compositeContent.messages[i].messageContent.digest(
                  compositeContent.messages[i]
                );
              }
            }
        ...
        if (fi.type === MessageContentType.File) {
          ...
          fi._fileIconName = helper.getFiletypeIcon(
            fi.title.substring(fi.title.lastIndexOf("."))
          );
      ...
      switch (favItem.type) {
        ...
        case MessageContentType.Video:
          store.previewMedia(
            favItem.url,
            favItem.thumbUrl,
            favItem.data && favItem.data.thumb
              ? favItem.data.thumb
              : "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNcunDhfwAGwgLoe4t2fwAAAABJRU5ErkJggg=="
          );
          break;
    ...
    handleClickMedia(index, favItems) {
      ...
      favItems = favItems.filter(
        (favItem) =>
          favItem.url || favItem.thumbUrl || (favItem.data && favItem.data.thumb)
      );
      favItems.forEach((favItem) => {
        let thumb = favItem.thumbUrl
          ? favItem.thumbUrl
          : favItem.data && favItem.data.thumb
          ? "data:image/png;base64," + favItem.data.thumb
          : "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNcunDhfwAGwgLoe4t2fwAAAABJRU5ErkJggg==";
        mediaItems.push({
          src: favItem.url,
          thumb: thumb,
          autoplay: true,
  ...
  computed: {
    ...
    filteredFavItems() {
      ...
      switch (this.category) {
        ...
        case "file":
          ...
          break;
        case "media":
          items = this.favItems.filter(
            (fi) =>
              fi.type === MessageContentType.Image || fi.type === MessageContentType.Video
          );
          break;
        case "composite":
          items = this.favItems.filter(
            (fi) => fi.type === MessageContentType.Composite_Message
          );
          break;
        default:
          break;
    ...
    groupedMediaItems() {
      let items = this.favItems.filter(
        (fi) =>
          fi.type === MessageContentType.Image || fi.type === MessageContentType.Video
      );
      let groupedItems = [];
      ...
      let months = [
        this.$t("common.month_1"),
        this.$t("common.month_1"),
        this.$t("common.month_2"),
        this.$t("common.month_3"),
        this.$t("common.month_4"),
        this.$t("common.month_5"),
        this.$t("common.month_6"),
        this.$t("common.month_7"),
        this.$t("common.month_8"),
        this.$t("common.month_9"),
        this.$t("common.month_10"),
        this.$t("common.month_11"),
        this.$t("common.month_12"),
      ];
