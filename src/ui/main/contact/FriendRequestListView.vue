<template>
  <section>
    <ul>
      <li v-for="(friendRequest,index) in sharedContactState.friendRequestList"
          :key="index"
          ...
          <div class="new-friend-item">
            ...
            <div class="info">
              <div class="name-action">
                <span class="name single-line">{{ mixinGetUserName(friendRequest._target )}}</span>
                <span v-if="friendRequest.status === 1"
                      class="status">{{$t('friend_request.accepted')}}</span>
                <button v-else-if="friendRequest.status === 0"
                        class="accept"
                        @click="accept(friendRequest)">{{$t('common.add')}}
                </button>
                <span v-else-if="friendRequest.status === 3"
                      class="status">{{$t('friend_request.denied')}}</span>
              </div>
              <p class="reason single-line">{{$t('friend_request.im') + mixinGetUserName(friendRequest._target ) }}</p>
            </div>
          </div>
        </div>
      </li>
