<template>
  <section class="channel-menu-container" id="channel-menu">
    <template v-for="(menu, index) in menus">
      ...
      >
        <div
          v-for="(sm, si) in menu.subMenus"
          :key="si"
          class="sub-menu-item"
          @click="openChannelMenu($event, sm)"
        >
          {{ sm.name }}
        </div>
      </tippy>
      <div :id="'menu_' + index" class="menu-item" @click="openChannelMenu($event, menu)">
        <p>
          {{ menuTile(menu) }}
        </p>
      </div>
    </template>
    <div @click="toggleMessageInput" class="bar">
      <img
        :alt="$t('channel.hide_menu')"
        :title="$t('channel.hide_menu')"
        draggable="false"
        src="../../../assets/images/keyboard-icon.svg"
      />
    </div>
  </section>
</template>
      ...
      switch (menu.type) {
        case "view":
          if (menu.url) {
            ...
            this.mixinGo2Web3View(menu.url);
          }
          break;
        case "click":
