    ...
    <div class="forward-message">
      <div v-if="forwardType === 0">
        <img
          ...
          alt=""
        />
        <p v-else>{{ forwardMessageStr }}</p>
      </div>
      <div v-else>
        <p>{{ forwardMessageStr }}</p>
      </div>
    </div>
    <label>
      <input
        type="text"
        :placeholder="$t('conversation.forward_extra')"
        v-model="extraMessageText"
      />
    </label>
  </section>
</template>
...
export default {
  ...
  created() {
    // 设置默认值
    const { messageContent } = this.messages[0];
    if (messageContent && messageContent.isPasswordNeed && messageContent.password) {
      this.extraMessageText = `${this.$t("voip.meeting_pw")}:${messageContent.password}`;
    }
  },
  computed: {
    forwardMessageStr() {
      ...
      switch (this.forwardType) {
        ...
        case ForwardType.ONE_BY_ONE:
          ...
          if (
            ...
            firstMsg.conversation.type === ConversationType.Single
          ) {
            str += this.$t("conversation.user_message_records", [
              firstMsg._from._displayName,
            ]);
          } else {
            if (this.forwardFrom !== "cloud") {
