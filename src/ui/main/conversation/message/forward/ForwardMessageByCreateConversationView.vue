    ...
    <section class="checked-user-list-container">
      ...
      <div class="content">
        <div class="picked-user-container"
             v-for="(user, index) in sharedPickState.users"
             :key="index">
          ...
          <span class="name new-single-line">{{ mixinGetUserName(user)}}</span>
        </div>
      </div>

      <ForwardMessageView ref="forwardMessageView"
                          v-if="sharedPickState.users.length > 0"
      ...
      <footer>
        ...
        <button @click="confirm"
                :class="['confirm',!sharedPickState.users.length?'disabled':'']">{{ $t('common.send') }}</button>
      </footer>
    </section>
  </div>
</template>
...
.checked-user-list-container footer button.confirm.disabled {
  opacity: 0.5;
}
</style>
