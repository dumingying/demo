<template>
    <div ref="userCardTippy" :id="'userCardInfoTrigger' + message.messageId" class="user-card-content-container">
        <div class="portrait-name-container">
            <img :src="message.messageContent.portrait" draggable="false" />
            <p>{{ message.messageContent.displayName }}</p>
        </div>
        <p class="desc single-line">{{ $t("message.card") }}</p>
        <tippy :to="'#userCardInfoTrigger' + message.messageId" interactive :animate-fill="false" placement="left"
            distant="7" theme="light" animation="fade" trigger="click">
            <template #content>
                <UserCardView @close="closeUserCard" :user-info="userInfo()" />
            </template>
        </tippy>
    </div>
</template>
    ...
    methods: {
        ...
        userInfo() {
            ...
            if (userCard.cardType === 0 || !userCard.cardType) {
                return wfc.getUserInfo(userCard.target, true);
            }
        },
    },
};
